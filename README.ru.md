...
| Сервер на Linux — обслуживание: проверка дюз и очистка головки Canon (raw-задание через CUPS) | ✅ подтверждено на MG2500 — распечатался шаблон дюз, слышна работа очистки головки |
| Сервер на Linux — обслуживание: пробная страница ОС | ⚠️ написано по документированному выводу утилит, на реальном железе не запускалось |
...
app/
  ...
  printing/
    ...
    maintenance.py     пробная страница / проверка дюз / очистка головки (задание Canon IVEC + BJL)
...
**Команды обслуживания для других марок.** В `app/printing/maintenance.py` лежит задание обслуживания для Canon. Добавьте проверку модели и сборку команд в том же стиле. Чтобы узнать, что отправляет драйвер производителя, поставьте на паузу задание, которое он создаёт для своей функции обслуживания, и прочитайте его через `ReadPrinter`. Так был получен формат Canon.
...
- **Пробная страница ОС** на сервере Linux (обслуживание) на реальном железе ещё не запускалась; проверка дюз/очистка головки Canon и сканирование (SANE) — запускались и работают.
