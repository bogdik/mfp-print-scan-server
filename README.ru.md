...
- Остальные форматы (`.txt`, `.docx`, …) печатает программа, зарегистрированная для этого типа файлов (Блокнот, Word/WordPad, …).
...
- **Проверка дюз** и **очистка головки** для струйных Canon: те же служебные команды BJL, что отправляет фильтр `commandtocanon` из [gutenprint](https://gimp-print.sourceforge.io/).
...
| Сервер на Linux — обслуживание (пробная страница / проверка дюз / очистка головки) | ⚠️ написано по документированному выводу утилит, на реальном железе не запускалось |
| Команды Canon «проверка дюз» / «очистка головки» | ⚠️ реализовано, на бумаге ещё не подтверждено |
| Автозапуск на Windows (`register_service.bat`) | ⚠️ фоновая работа с логом в файл проверена; саму регистрацию задачи (нужны права администратора) ещё не подтверждали |
| Unit-файл systemd для Linux | ⚠️ пример, на реальной системе не запускался |
...
| TXT, DOCX, RTF, … | программа, зарегистрированная для печати этого типа (Windows), или LibreOffice → PDF (Linux) | — |
...
|---|---|---|
| Пробная страница | любые | стандартная пробная страница ОС (`Win32_Printer.PrintTestPage` / `testprint` из CUPS) |
...
app/
  ...
  printing/
    ...
    linux_cups.py      бэкенд Linux (lp, lpoptions, конвертация через LibreOffice)
    ...
    maintenance.py     пробная страница / проверка дюз / очистка головки (Canon BJL)
...
Без Microsoft Word Windows печатает `.docx` через WordPad, который теряет форматирование. Сохраните документ в PDF на клиенте или установите Word либо LibreOffice.
</details>
...
**Команды обслуживания для других марок.** В `app/printing/maintenance.py` лежат команды BJL для Canon. Добавьте проверку модели и таблицу команд в том же стиле.
...
- **Офисные форматы на Windows** печатает программа, зарегистрированная для них; без Word `.docx` теряет форматирование.
...
- **Серверная часть на Linux** для обслуживания (пробная страница / проверка дюз / очистка головки) на реальном железе ещё не запускалась; сканирование (SANE) — запускалось и работает.
...
- [gutenprint](https://gimp-print.sourceforge.io/) `commandtocanon`: служебные команды BJL для Canon.
